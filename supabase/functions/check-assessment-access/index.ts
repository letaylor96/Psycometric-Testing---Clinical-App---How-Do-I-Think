...
serve(async (req) => {
  ...
  try {
    ...
    // === INPUT VALIDATION ===
    const VALID_ASSESSMENT_TYPES = ['personality', 'iq', 'cognitive', 'adhd', 'bundle'] as const;
    ...
    if (!VALID_ASSESSMENT_TYPES.includes(assessmentType as typeof VALID_ASSESSMENT_TYPES[number])) {
      return new Response(
        JSON.stringify({ error: 'Invalid assessment type. Must be one of: personality, iq, cognitive, adhd, bundle' }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
